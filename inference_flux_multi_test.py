...
def generate_output_dir_name(model_name, prompt_levels, color_levels, split, max_samples_per_dir, lora_weights=""):
    ...
    Format: {model_short_name}_{lora_name}_P{prompt_levels}_C{color_levels}_{split}_{samples}
    
    Examples:
      - Qwen-Image_P1_C3_test_1000
      - Qwen-Image_lora_prompt_split_result_ckpt3000_P1-2_C1-2-3_test_full
    """
    ...
    lora_flag = ""
    if lora_weights:
        # Try to extract name after "lora_saves_"
        if "lora_saves_" in lora_weights:
            # Extract part after "lora_saves_"
            lora_part = lora_weights.split("lora_saves_")[-1]
            # Get directory name (before /checkpoint or /pytorch)
            lora_name = lora_part.split("/")[0]
            # Try to get checkpoint number
